...
class CourtAvailabilityService
{
    public function isCourtAvailable(
        ...
        $court->load([
            'complex.specialDates' => fn($q) => $q->whereDate('date', $startAt->toDateString()),
            'complex.openingHours',
            'blocks' => fn($q) => $q->where('start_at', '<', $endAt)->where('end_at', '>', $startAt),
            'reservations' => fn($q) => $q->when($ignoreReservationId, fn($sq) => $sq->whereKeyNot($ignoreReservationId))
                ->whereIn('status', [Reservation::STATUS_PENDIENTE_PAGO, Reservation::STATUS_CONFIRMADA])
